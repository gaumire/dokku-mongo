...
@test "($PLUGIN_COMMAND_PREFIX:import) error import without database name" {
...
@test "($PLUGIN_COMMAND_PREFIX:import) success" {
  ...
  run dokku "$PLUGIN_COMMAND_PREFIX:import" l l < "$PLUGIN_DATA_ROOT/fake.dump.tar"
  password="$(cat "$PLUGIN_DATA_ROOT/l/dbs/l/PASSWORD")"
  user="$(cat "$PLUGIN_DATA_ROOT/l/dbs/l/USER")"
  assert_output "docker exec -i dokku.mongo.l bash -c DIR=\$(mktemp -d) && tar xf - -C \"\$DIR\" && mongorestore -d l -u \"$user\" -p \"$password\" --authenticationDatabase \"l\" \$(find \"\$DIR\" -mindepth 1 -maxdepth 1 -type d | head -n1) && rm -rf \"\$DIR\""
}
