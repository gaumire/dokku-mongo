...
@test "($PLUGIN_COMMAND_PREFIX:promote) changes MONGO_URL" {
  ...
  dokku config:set my_app "MONGO_URL=mongodb://u:p@host:27017/db" "DOKKU_MONGO_BLUE_URL=mongodb://$user:$password@dokku-mongo-l:27017/l"
  dokku "$PLUGIN_COMMAND_PREFIX:promote" l my_app l
  url=$(dokku config:get my_app MONGO_URL)
  assert_equal "$url" "mongodb://$user:$password@dokku-mongo-l:27017/l"
}
...
@test "($PLUGIN_COMMAND_PREFIX:promote) uses MONGO_DATABASE_SCHEME variable" {
  local password="$(cat "$PLUGIN_DATA_ROOT/l/dbs/l/PASSWORD")"
  local user="$(cat "$PLUGIN_DATA_ROOT/l/dbs/l/USER")"
  dokku config:set my_app "MONGO_DATABASE_SCHEME=mongodb2" "MONGO_URL=mongodb://u:p@host:27017/db" "DOKKU_MONGO_BLUE_URL=mongodb2://$user:$password@dokku-mongo-l:27017/l"
  dokku "$PLUGIN_COMMAND_PREFIX:promote" l my_app l
  url=$(dokku config:get my_app MONGO_URL)
